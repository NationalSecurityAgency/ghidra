	...
	String parseLine(String line) {
		...
		try {
			...
			if (!record.isReportedChecksumCorrect()) {
				msg =
					"WARNING: line checksum (is " + record.getReportedChecksum() +
...
//                log(line, "INVALID RECORD TYPE " + record.getRecordType() + ", offset=" + loadOffset + ", length=" + data.length);
	...
	String createMemory(String creator, String progFile, String blockName, boolean isOverlay,
		...
		try {
			...
			//in address order...
			int count = 0;
			AddressSetPartitioner partitioner =
				new AddressSetPartitioner(set, rangeMap, partitions);
			...
			for (AddressRange blockRange : partitioner) {
				...
				for (AddressRange range : blockSet) {
					...
					int pos =
						(int) range.getMinAddress().getOffset() -
				...
				if (isOverlay) {
					...
					while (true) {
						try {
							mbu.createOverlayBlock(name, blockRange.getMinAddress(),
								new ByteArrayInputStream(data), data.length,
								"Generated by " + creator, progFile, true, false, false, monitor);
							break;
						}
					...
					mbu.createInitializedBlock(base.getAddressSpace().getName(),
						blockRange.getMinAddress(), data, "Generated by " + creator, progFile,
