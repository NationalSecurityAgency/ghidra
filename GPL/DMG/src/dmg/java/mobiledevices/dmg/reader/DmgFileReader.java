...
public class DmgFileReader implements Closeable {
	...
	private void openPartition(Partition selectedPartition) throws IOException {
		...
		if (fsType == FileSystemRecognizer.FileSystemType.HFS_PLUS || fsType == FileSystemRecognizer.FileSystemType.HFSX
			...
			// TODO possible null dereferencing here
			...
			if (factory.isSupported(StandardAttribute.CACHING_ENABLED)) {
				factory.getCreateAttributes().setBooleanAttribute(StandardAttribute.CACHING_ENABLED, true);
			}

			ReadableRandomAccessStream stage1;

			if (fsLength > 0) {
				stage1 = new ReadableConcatenatedStream(this.rras, fsOffset, fsLength);
			} else {
				stage1 = this.rras;
			}
			...
			if (fileSystemHandler instanceof HFSXFileSystemHandler) {
				this.parser = new AttributesFileParser((HFSXFileSystemHandler) fileSystemHandler,
		...
		if (entry != null && entry.isFile()) {
			...
			if (mainFork.getLength() > 0) {
				...
				if (decmpfsHeader.getCompressionType() == DecmpfsCompressionTypes.CMP_Type3) {
					...
					InputStream inputStream = new ByteArrayInputStream(decmpfsHeader.getAttrBytes());

					ByteArrayOutputStream uncompressedBytes = zlib.decompress(inputStream,
							(int) decmpfsHeader.getUncompressedSize());

					File tempDecompressedFile = GFileUtilityMethods.writeTemporaryFile(uncompressedBytes.toByteArray(),
							entry.getName());
	...
	/**
	 * Returns the length of the given file system entry. If the entry is actually a
	 * directory, then -1 is returned.
	 */
	public long getLength(FSEntry entry) {
		if (entry != null & entry.isFile()) {
			...
			try {
				if (this.parser != null) {
					DecmpfsHeader header = this.parser.getDecmpfsHeader(entry.asFile());
					if (header != null) {
	 ...
	 *
	 * Note: the "a" will be stripped because it corresponds to the file system
	 * handler.
	 */
	...
	/**
	 * Returns the DMG file object for the corresponding path. Path should contain
	 * the file system handler name.
	 */
