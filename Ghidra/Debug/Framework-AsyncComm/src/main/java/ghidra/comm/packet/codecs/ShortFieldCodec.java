...
/**
 * The {@link FieldCodec} for encoding short values
 * 
 ...
 */
public class ShortFieldCodec<ENCBUF, DECBUF> extends AbstractFieldCodec<ENCBUF, DECBUF, Short> {
	private final PrimitiveCodec<ENCBUF, DECBUF> codec;

	public ShortFieldCodec(PrimitiveCodec<ENCBUF, DECBUF> codec, Class<? extends Packet> pktType,
			Field field) {
		super(pktType, field);
		this.codec = codec;
	}
	...
	@Override
	public Short decodeField(Packet pkt, DECBUF buf, Integer count, PacketFactory factory)
			throws IOException, PacketDecodeException {
		return codec.decodeShort(field, buf);
	}

	@Override
	public void encodeField(ENCBUF buf, Packet pkt, Short val)
			throws IOException, PacketEncodeException {
		codec.encodeShort(buf, field, val);
	}
}
