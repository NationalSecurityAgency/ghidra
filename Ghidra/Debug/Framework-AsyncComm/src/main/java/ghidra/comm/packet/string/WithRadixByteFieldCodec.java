...
/**
 * The {@link FieldCodec} for {@code byte} fields annotated by {@link WithRadix}
 * 
 * @see WithRadixWrapperFactory
 */
public class WithRadixByteFieldCodec extends AbstractFieldCodec<StringBuilder, CharBuffer, Byte> {
	private final int radix;
	private final StringPacketCodec strCodec;
	private final FieldCodec<StringBuilder, CharBuffer, String> chainNext;

	public WithRadixByteFieldCodec(Class<? extends Packet> pktType, Field field, int radix,
	...
	@Override
	public Byte decodeField(Packet pkt, CharBuffer buf, Integer count, PacketFactory factory)
			throws IOException, PacketDecodeException {
		int len = StringPacketCodec.computeMaxChars(radix, 1);
		CharBuffer part = CharBuffer.wrap(chainNext.decodeField(pkt, buf, len, factory));
		Byte result = strCodec.decodeByte(field, part, radix);
		buf.position(buf.position() - part.remaining());
		return result;
	}

	@Override
	public void encodeField(StringBuilder buf, Packet pkt, Byte val)
			throws IOException, PacketEncodeException {
		StringBuilder part = strCodec.newEncodeBuffer();
		strCodec.encodeByte(part, field, val, radix);
