...
/**
 * The {@link FieldCodec} for encoding character values
 * 
 ...
 */
public class CharacterFieldCodec<ENCBUF, DECBUF>
		extends AbstractFieldCodec<ENCBUF, DECBUF, Character> {
	private final PrimitiveCodec<ENCBUF, DECBUF> codec;

	public CharacterFieldCodec(PrimitiveCodec<ENCBUF, DECBUF> codec,
	...
	@Override
	public Character decodeField(Packet pkt, DECBUF buf, Integer count, PacketFactory factory)
			throws IOException, PacketDecodeException {
		return codec.decodeChar(field, buf);
	}

	@Override
	public void encodeField(ENCBUF buf, Packet pkt, Character val)
			throws IOException, PacketEncodeException {
		codec.encodeChar(buf, field, val);
	}
}
