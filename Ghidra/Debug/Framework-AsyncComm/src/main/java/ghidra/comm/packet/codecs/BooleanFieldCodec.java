...
/**
 * The {@link FieldCodec} for encoding boolean values
 * 
 ...
 */
public class BooleanFieldCodec<ENCBUF, DECBUF> extends AbstractFieldCodec<ENCBUF, DECBUF, Boolean> {
	private final PrimitiveCodec<ENCBUF, DECBUF> codec;

	public BooleanFieldCodec(PrimitiveCodec<ENCBUF, DECBUF> codec, Class<? extends Packet> pktType,
			Field field) {
		super(pktType, field);
		this.codec = codec;
	}
	...
	@Override
	public Boolean decodeField(Packet pkt, DECBUF buf, Integer count, PacketFactory factory)
			throws IOException, PacketDecodeException {
		return codec.decodeBoolean(field, buf);
	}

	@Override
	public void encodeField(ENCBUF buf, Packet pkt, Boolean val)
			throws IOException, PacketEncodeException {
		codec.encodeBoolean(buf, field, val);
	}
}
