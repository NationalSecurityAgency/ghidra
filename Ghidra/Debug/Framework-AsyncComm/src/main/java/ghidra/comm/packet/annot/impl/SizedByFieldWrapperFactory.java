...
public class SizedByFieldWrapperFactory<ENCBUF, DECBUF>
	...
	public <T> FieldCodec<ENCBUF, DECBUF, T> getWrappedFieldCodecForType(
		...
		SizedByField annot = field.getAnnotation(SizedByField.class);
		/*
		...
		final Field sizingField;
		try {
			sizingField = pktType.getField(annot.value());
		}
		catch (NoSuchFieldException | SecurityException e) {
			throw new InvalidFieldNameException(pktType, field, annot, e.getMessage(), e);
		}
		if (!isIntegralType(sizingField.getType())) {
			throw new InvalidFieldNameException(pktType, sizingField, annot,
				"Field is not an integral type");
		}

		SizedByFieldInjectionFactory<ENCBUF, DECBUF> injected =
			new SizedByFieldInjectionFactory<>(field, annot.adjust());
		codec.injectChain(pktType, sizingField, injected, field, annot);

		FieldCodec<ENCBUF, DECBUF, T> chainNext =
			FieldCodecFactory.buildChainForType(pktType, field, fldType, codec, chain);
		return new SizedByFieldFieldCodec<>(pktType, field, sizingField, annot, codec, chainNext);
	}
}
