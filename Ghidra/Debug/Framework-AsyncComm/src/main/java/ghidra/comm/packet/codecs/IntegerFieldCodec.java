...
/**
 * The {@link FieldCodec} for encoding integer values
 * 
 ...
 */
public class IntegerFieldCodec<ENCBUF, DECBUF> extends AbstractFieldCodec<ENCBUF, DECBUF, Integer> {
	private final PrimitiveCodec<ENCBUF, DECBUF> codec;

	public IntegerFieldCodec(PrimitiveCodec<ENCBUF, DECBUF> codec, Class<? extends Packet> pktType,
			Field field) {
		super(pktType, field);
		this.codec = codec;
	}
	...
	@Override
	public Integer decodeField(Packet pkt, DECBUF buf, Integer count, PacketFactory factory)
			throws IOException, PacketDecodeException {
		return codec.decodeInt(field, buf);
	}

	@Override
	public void encodeField(ENCBUF buf, Packet pkt, Integer val)
			throws IOException, PacketEncodeException {
		codec.encodeInt(buf, field, val);
	}
}
