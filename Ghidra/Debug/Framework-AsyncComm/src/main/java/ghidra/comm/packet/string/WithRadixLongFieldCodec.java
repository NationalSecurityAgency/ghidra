...
/**
 * The {@link FieldCodec} for {@code long} fields annotated by {@link WithRadix}
 * 
 * @see WithRadixWrapperFactory
 */
public class WithRadixLongFieldCodec extends AbstractFieldCodec<StringBuilder, CharBuffer, Long> {
	private final int radix;
	private final StringPacketCodec strCodec;
	private final FieldCodec<StringBuilder, CharBuffer, String> chainNext;

	public WithRadixLongFieldCodec(Class<? extends Packet> pktType, Field field, int radix,
	...
	@Override
	public Long decodeField(Packet pkt, CharBuffer buf, Integer count, PacketFactory factory)
			throws IOException, PacketDecodeException {
		int len = StringPacketCodec.computeMaxChars(radix, 8);
		CharBuffer part = CharBuffer.wrap(chainNext.decodeField(pkt, buf, len, factory));
		Long result = strCodec.decodeLong(field, part, radix);
		buf.position(buf.position() - part.remaining());
		return result;
	}

	@Override
	public void encodeField(StringBuilder buf, Packet pkt, Long val)
			throws IOException, PacketEncodeException {
		StringBuilder part = strCodec.newEncodeBuffer();
		strCodec.encodeLong(part, field, val, radix);
