...
/**
 * The {@link FieldCodec} for encoding single-precision float values
 * 
 ...
 */
public class FloatFieldCodec<ENCBUF, DECBUF> extends AbstractFieldCodec<ENCBUF, DECBUF, Float> {
	private final PrimitiveCodec<ENCBUF, DECBUF> codec;

	public FloatFieldCodec(PrimitiveCodec<ENCBUF, DECBUF> codec, Class<? extends Packet> pktType,
			Field field) {
		super(pktType, field);
		this.codec = codec;
	}
	...
	@Override
	public Float decodeField(Packet pkt, DECBUF buf, Integer count, PacketFactory factory)
			throws IOException, PacketDecodeException {
		return codec.decodeFloat(field, buf);
	}

	@Override
	public void encodeField(ENCBUF buf, Packet pkt, Float val)
			throws IOException, PacketEncodeException {
		codec.encodeFloat(buf, field, val);
	}
}
