...
/**
 * The {@link FieldCodec} for encoding byte values
 * 
 ...
 */
public class ByteFieldCodec<ENCBUF, DECBUF> extends AbstractFieldCodec<ENCBUF, DECBUF, Byte> {
	private final PrimitiveCodec<ENCBUF, DECBUF> codec;

	public ByteFieldCodec(PrimitiveCodec<ENCBUF, DECBUF> codec, Class<? extends Packet> pktType,
			Field field) {
		super(pktType, field);
		this.codec = codec;
	}
	...
	@Override
	public Byte decodeField(Packet pkt, DECBUF buf, Integer count, PacketFactory factory)
			throws IOException, PacketDecodeException {
		return codec.decodeByte(field, buf);
	}

	@Override
	public void encodeField(ENCBUF buf, Packet pkt, Byte val)
			throws IOException, PacketEncodeException {
		codec.encodeByte(buf, field, val);
	}
}
