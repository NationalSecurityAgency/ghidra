...
/**
 * The factory injecting {@link SizedByMethodsInjectedFieldCodec} on behalf of
 * {@link SizedByMethods}
 *
 * @param <ENCBUF> the type of temporary buffer used to encode
 * @param <DECBUF> the type of temporary buffer used to decode
 */
public class SizedByMethodsInjectionFactory<ENCBUF, DECBUF>
	...
	public SizedByMethodsInjectionFactory(Field sizedField, Method getter, Method setter) {
	...
	public <T> FieldCodec<ENCBUF, DECBUF, T> getWrappedFieldCodecForType(
		...
		return new SizedByMethodsInjectedFieldCodec<>(pktType, field, sizedField, getter, setter,
			codec, chainNext);
	}
}
