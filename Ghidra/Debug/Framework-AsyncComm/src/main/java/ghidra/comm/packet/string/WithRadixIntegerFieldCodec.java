...
/**
 * The {@link FieldCodec} for {@code int} fields annotated by {@link WithRadix}
 * 
 * @see WithRadixWrapperFactory
 */
public class WithRadixIntegerFieldCodec
		extends AbstractFieldCodec<StringBuilder, CharBuffer, Integer> {
	private final int radix;
	private final StringPacketCodec strCodec;
	private final FieldCodec<StringBuilder, CharBuffer, String> chainNext;

	public WithRadixIntegerFieldCodec(Class<? extends Packet> pktType, Field field, int radix,
	...
	@Override
	public Integer decodeField(Packet pkt, CharBuffer buf, Integer count, PacketFactory factory)
			throws IOException, PacketDecodeException {
		int len = StringPacketCodec.computeMaxChars(radix, 4);
		CharBuffer part = CharBuffer.wrap(chainNext.decodeField(pkt, buf, len, factory));
		Integer result = strCodec.decodeInt(field, part, radix);
		buf.position(buf.position() - part.remaining());
		return result;
	}

	@Override
	public void encodeField(StringBuilder buf, Packet pkt, Integer val)
			throws IOException, PacketEncodeException {
		StringBuilder part = strCodec.newEncodeBuffer();
		strCodec.encodeInt(part, field, val, radix);
