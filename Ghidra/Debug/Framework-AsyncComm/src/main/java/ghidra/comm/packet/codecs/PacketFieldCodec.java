...
/**
 * The {@link FieldCodec} for encoding sub-packet values
 * 
 ...
 */
public class PacketFieldCodec<ENCBUF, DECBUF> extends AbstractFieldCodec<ENCBUF, DECBUF, Packet> {
	private final PrimitiveCodec<ENCBUF, DECBUF> codec;
	private final Class<? extends Packet> type;

	public PacketFieldCodec(PrimitiveCodec<ENCBUF, DECBUF> codec, Class<? extends Packet> pktType,
			Field field, Class<? extends Packet> type) {
		super(pktType, field);
		this.codec = codec;
		this.type = type;
	}
	...
	@Override
	public Packet decodeField(Packet pkt, DECBUF buf, Integer count, PacketFactory factory)
			throws IOException, PacketDecodeException {
		return codec.decodePacket(pkt, field, buf, type, factory);
	}

	@Override
	public void encodeField(ENCBUF buf, Packet pkt, Packet val)
			throws IOException, PacketEncodeException {
		codec.encodePacket(buf, field, val);
	}
}
