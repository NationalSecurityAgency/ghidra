...
/**
 * An annotation that indicates a field is terminated by a sequence of bytes
 * 
 * This annotation is only meaningful to byte-based packet codecs. When encoded, the annotated field
 * will be conditionally followed by the byte sequence. When decoded, the field codec will read
 * ahead to the terminator and limit the chained codec to the preceding data. Once decoded, it will
 * resume immediately following the terminator.
 * 
 * @see NullTerminated
 */
@Retention(RetentionPolicy.RUNTIME)
@Target(ElementType.FIELD)
public @interface SequenceTerminated {
	/**
