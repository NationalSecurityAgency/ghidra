...
/**
 * The {@link FieldCodec} for fields annotated by {@link SizedByMethods}
 *
 ...
 */
class SizedByMethodsFieldCodec<ENCBUF, DECBUF, T> extends AbstractFieldCodec<ENCBUF, DECBUF, T> {
	private final PacketCodecInternal<?, ENCBUF, DECBUF> codec;
	private final FieldCodec<ENCBUF, DECBUF, T> chainNext;
	private final Method getter;

	SizedByMethodsFieldCodec(Class<? extends Packet> pktType, Field field, Method getter,
	...
	public T decodeField(Packet pkt, DECBUF buf, Integer count, PacketFactory factory)
		...
		try {
			int len = (int) getIntegralValue(getter.invoke(pkt));
