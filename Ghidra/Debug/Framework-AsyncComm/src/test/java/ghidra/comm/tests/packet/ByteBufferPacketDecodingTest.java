...
public class ByteBufferPacketDecodingTest implements PacketTestClasses {
	protected PacketCodec<ByteBuffer> codec; // Use the interface to ensure methods are defined there
	...
	public static ByteBuffer consPacket(String... parts) {
		String full = StringUtils.join(parts).replace(":", "");
		return ByteBuffer.wrap(NumericUtilities.convertStringToBytes(full));
	}
	...
	// Often have to copy, since decode consumes the buffer in place
	public ByteBuffer copy(ByteBuffer src) {
		return ByteBuffer.wrap(src.array());
	}
	...
	public void testTypedAnnot() throws PacketDecodeException, FieldOrderingException {
		...
		TestMessageDynamicTyped p1 = codec.decodePacket(TestMessageDynamicTyped.class, copy(enc));
		...
		p1 = codec.decodePacket(TestMessageDynamicTyped.class, copy(enc));
	...
	@Test
	public void testAbstractElements() throws PacketDecodeException {
	...
	@Test
	public void testAbstractPacket() throws PacketDecodeException {
		PacketTestClasses.LONG_TEST_NUMBER_FACTORY.registerTypes(codec);
		PacketTestClasses.INT_TEST_NUMBER_FACTORY.registerTypes(codec);
		ByteBuffer enc;

		enc = consPacket("00:00:00:00:00:00:00:02");
		AbstractTestNumber p1 = codec.decodePacket(AbstractTestNumber.class, enc,
			PacketTestClasses.LONG_TEST_NUMBER_FACTORY);
		AbstractTestNumber e1 = new LongTestNumber(2);
		assertEquals(e1, p1);

		enc = consPacket("00:00:00:02");
		AbstractTestNumber p2 = codec.decodePacket(AbstractTestNumber.class, enc,
			PacketTestClasses.INT_TEST_NUMBER_FACTORY);
		AbstractTestNumber e2 = new IntTestNumber(2);
		assertEquals(e2, p2);
	}
