...
public class OpBehaviorIntSLessTest extends AbstractOpBehaviorTest {
	static final OpBehaviorIntSlessEqual OP = OpBehavior.INT_SLESSEQUAL;
	...
	public void testEvaluateBinaryBigInteger() {
		Assert.assertEquals(BigInteger.ONE,
			OP.evaluateBinary(1, 16, BigInteger.ZERO, BigInteger.ZERO));
		Assert.assertEquals(BigInteger.ZERO,
			OP.evaluateBinary(1, 16, BigInteger.ONE, BigInteger.ZERO));
		Assert.assertEquals(BigInteger.ONE,
			OP.evaluateBinary(1, 16, BigInteger.ZERO, BigInteger.ONE));
		Assert.assertEquals(BigInteger.ONE,
			OP.evaluateBinary(1, 16, NEGATIVE_ONE, BigInteger.ZERO));
		Assert.assertEquals(BigInteger.ZERO,
			OP.evaluateBinary(1, 16, BigInteger.ZERO, NEGATIVE_ONE));
		Assert.assertEquals(BigInteger.ONE, OP.evaluateBinary(1, 16, NEGATIVE_ONE, BigInteger.ONE));
		Assert.assertEquals(BigInteger.ZERO,
			OP.evaluateBinary(1, 16, BigInteger.ONE, NEGATIVE_ONE));
		Assert.assertEquals(BigInteger.ONE, OP.evaluateBinary(1, 16, NEGATIVE_ONE, NEGATIVE_ONE));
		Assert.assertEquals(BigInteger.ONE,
			OP.evaluateBinary(1, 16, BIG_NEGATIVE, BigInteger.ZERO));
		Assert.assertEquals(BigInteger.ZERO,
			OP.evaluateBinary(1, 16, BigInteger.ZERO, BIG_NEGATIVE));
		Assert.assertEquals(BigInteger.ONE, OP.evaluateBinary(1, 16, BIG_NEGATIVE, BigInteger.ONE));
		Assert.assertEquals(BigInteger.ZERO,
			OP.evaluateBinary(1, 16, BigInteger.ONE, BIG_NEGATIVE));
