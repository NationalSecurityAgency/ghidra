 ...
 * together form the "sequence number." Because the executor cares not about the derivation of a
 ...
 * <p>
 * Methods receiving a frame should use it only for the userop names, usually to fabricate a sub
 * frame. Other than the executor, things should generally not access its code. If a {@link PcodeOp}
 * is provided to the same method, generally that is the only op this frame is guaranteed to
 * contain. The execution engine <em>is not</em> required to provide the surrounding ops.
 */
public class PcodeFrame {
	protected class MyAppender extends AbstractAppender<String> {
		...
		public void appendIndent() {
			...
			SequenceNumber seq = code.get(i).getSeqnum();
			buf.append(seq.getTarget()).append(".").append(seq.getTime()).append(": ");
		}
		...
		protected String stringifyUseropUnchecked(Language language, int id) {
			...
			return userops.getUseropName(id);
		}
	...
	private final PcodeUseropSymbolMap userops;
	 ...
	 * @param userops a map of additional sleigh/p-code userops linked to the program
	 */
	public PcodeFrame(Language language, List<PcodeOp> code, PcodeUseropSymbolMap userops) {
	...
	public String getUseropName(int userop) {
		return userops.getUseropName(userop);
	}

	/**
	 * {@return the map of userop symbols}
	 */
	public PcodeUseropSymbolMap getUserops() {
		return userops;
	}
