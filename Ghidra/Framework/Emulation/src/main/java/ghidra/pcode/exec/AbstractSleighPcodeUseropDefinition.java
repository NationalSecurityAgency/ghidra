 ...
 */
public abstract class AbstractSleighPcodeUseropDefinition implements SleighPcodeUseropDefinition {
	...
	public static class Builder implements BuilderStage1 {
		 ...
		 */
		@Override
		public SleighPcodeUseropDefinition build() {
			overload();
			if (definitions.size() == 1) {
				return new FixedSleighPcodeUseropDefinition(name,
					definitions.values().iterator().next());
			}
			return new OverloadedSleighPcodeUseropDefinition(name, Map.copyOf(definitions));
		}
	}
	...
	protected AbstractSleighPcodeUseropDefinition(String name) {
		this.name = name;
	}
	...
	public void execute(PcodeExecutor<Object> executor, PcodeUseropLibrary<Object> library,
		...
		args.add(outArg);
		args.addAll(inArgs);
		PcodeProgram program = programFor(executor.language, args, library);
