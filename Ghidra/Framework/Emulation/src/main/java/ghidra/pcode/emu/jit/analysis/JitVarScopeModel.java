...
public class JitVarScopeModel {
	...
	enum Which {
		...
		UP {
			@Override
			Collection<JitBlock> getFlows(ScopeInfo info, JitReachabilityModel rm) {
				return info.block.flowsTo()
						.values()
						.stream()
		 ...
		 * @param rm the reachability model, as only "taken" flows will be followed
		 * @return the blocks into which our set will be unioned
		 */
		abstract Collection<JitBlock> getFlows(ScopeInfo info, JitReachabilityModel rm);
	...
	private class ScopeInfo {
		...
		private void push(Which which) {
			...
			for (JitBlock block : which.getFlows(this, rm)) {
