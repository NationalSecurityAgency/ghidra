 ...
 */
public class DataTypeArchiveContentHandler extends DBContentHandler<ProjectDtArchiveDB> {
	...
	final static Class<ProjectDtArchiveDB> DATA_TYPE_ARCHIVE_DOMAIN_OBJECT_CLASS =
		ProjectDtArchiveDB.class;
	...
	public long createFile(FileSystem fs, FileSystem userfs, String path, String name,
		...
		if (!(obj instanceof ProjectDtArchiveDB)) {
			throw new IOException("Unsupported domain object: " + obj.getClass().getName());
		}
		return createFile((ProjectDtArchiveDB) obj, DATA_TYPE_ARCHIVE_CONTENT_TYPE, fs, path,
			name,
			monitor);
	}

	@Override
	public ProjectDtArchiveDB getImmutableObject(FolderItem item, Object consumer, int version,
		...
		DBHandle dbh = null;
		ProjectDtArchiveDB dataTypeArchive = null;
		...
		try {
			...
			dataTypeArchive =
				new ProjectDtArchiveDB(dbh, name, OpenMode.IMMUTABLE, monitor, consumer);
	...
	@Override
	public ProjectDtArchiveDB getReadOnlyObject(FolderItem item, int version,
			boolean okToUpgrade,
		...
		try {
			...
			dataTypeArchive = new ProjectDtArchiveDB(dbh, name, openMode, monitor, consumer);
			getDataTypeArchiveChangeSet(dataTypeArchive, bf);
			success = true;
			return dataTypeArchive;
		}
	...
	@Override
	public ProjectDtArchiveDB getDomainObject(FolderItem item, FileSystem userfs,
			long checkoutId,
	...
	private boolean recoverChangeSet(ProjectDtArchiveDB dataTypeArchive, DBHandle dbh)
			throws IOException {
	...
	private DataTypeArchiveDBChangeSet getDataTypeArchiveChangeSet(
			ProjectDtArchiveDB dataTypeArchive, ManagedBufferFile bf) throws IOException {
		DataTypeArchiveDBChangeSet changeSet =
			(DataTypeArchiveDBChangeSet) dataTypeArchive.getChangeSet();
	...
	public ChangeSet getChangeSet(FolderItem item, int fromVer, int toVer)
		...
		try {
			...
			dbh = new DBHandle(bf);
			dataTypeArchive =
				new ProjectDtArchiveDB(dbh, "tmp", OpenMode.IMMUTABLE, null, this);
	...
	@Override
	public Class<ProjectDtArchiveDB> getDomainObjectClass() {
	...
	public DomainObjectMergeManager getMergeManager(DomainObject resultsObj, DomainObject sourceObj,
		...
		return DataTypeArchiveMergeManagerFactory.getMergeManager(
			(ProjectDataTypeArchive) resultsObj, (ProjectDataTypeArchive) sourceObj,
			(ProjectDataTypeArchive) originalObj, (ProjectDataTypeArchive) latestObj);
	}
	...
	public void resetDBSourceFile(FolderItem item, DomainObjectAdapterDB domainObj)
		...
		if (!(item instanceof LocalDatabaseItem dbItem) ||
			!(domainObj instanceof ProjectDtArchiveDB dataTypeArchive)) {
