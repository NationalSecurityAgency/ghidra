...
public interface Array extends DataType {
	...
	/**
	 * Get the StringDataInstance object which corresponds to an array in memory.  This will either be a
	 * StringDataInstrance for the ArrayStringable case or null.
	 * 
	 * @param buf data buffer
	 * @param settings data settings
	 * @param length length of array
	...
	default StringDataInstance getStringDataInstance(MemBuffer buf, Settings settings, int length) {
		...
		return (as != null) ? as.getArrayStringDataInstance(buf, settings, length) : null;
	}
