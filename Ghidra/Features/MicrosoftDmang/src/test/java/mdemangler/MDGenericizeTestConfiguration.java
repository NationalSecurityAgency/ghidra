 ...
 *  of driving the tests of MDMangGenericizeTest.
 */
public class MDGenericizeTestConfiguration extends MDBaseTestConfiguration {

	public MDGenericizeTestConfiguration(boolean quiet) {
		super(quiet);
		mdm = new MDMangGenericize();
	}

	@Override
	protected void doExtraProcCheck() throws Exception {
		if (demangItem == null) {
			outputInfo.append("GDemang: N/A");
			return;
		}
		String mangledGeneric = ((MDMangGenericize) mdm).getGenericSymbol();
		outputInfo.append(getNumberHeader(mangledGeneric.length()));
		outputInfo.append("\nGeneric: ");
