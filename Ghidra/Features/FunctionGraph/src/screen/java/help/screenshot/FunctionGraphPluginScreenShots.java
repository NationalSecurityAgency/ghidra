...
//		createCalloutOnImage(parentImage, parentComponent, calloutInfo);
	...
	protected GroupedFunctionGraphVertex group(Set<FGVertex> ungroupedVertices,
		...
		final MultiLineInputDialog dialog =
			waitForDialogComponent(tool.getToolFrame(), MultiLineInputDialog.class, 3000);
	...
	private DialogComponentProvider showGroupTextDialog(FGVertex... vertices) {
		...
		return waitForDialogComponent(tool.getToolFrame(), MultiLineInputDialog.class, 3000);
	}
	...
	private void setLayout() {
		...
		for (Object state : states) {
			if (((ActionState) state).getName().indexOf("Min Cross") != -1) {
				minCrossState = state;
				break;
			}
		}
