...
/// \brief Pure-read precondition mirror.  Needs at least 2 same-opcode descendants of input(0).
int4 RuleSelectCse::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *vn = op->getIn(0);
  OpCode opc = op->code();
  int4 sameOpcCount = 0;
  for (list<PcodeOp *>::const_iterator iter = vn->beginDescend(); iter != vn->endDescend(); ++iter) {
    const PcodeOp *other = *iter;
    if (other->code() != opc) continue;
    if (other->getCseHash() == 0) continue;
    sameOpcCount += 1;
    if (sameOpcCount >= 2) return 1;
  }
  return 0;
}
...
int4 RulePiece2Zext::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *constvn = op->getIn(0);
  return (constvn->isConstant() && constvn->getOffset() == 0) ? 1 : 0;
}
...
/// \brief Pure-read precondition mirror.
int4 RulePiece2Sext::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *shiftout = op->getIn(0);
  if (!shiftout->isWritten()) return 0;
  const PcodeOp *shiftop = shiftout->getDef();
  if (shiftop->code() != CPUI_INT_SRIGHT) return 0;
  if (!shiftop->getIn(1)->isConstant()) return 0;
  int4 n = shiftop->getIn(1)->getOffset();
  const Varnode *x = shiftop->getIn(0);
...
/// \brief Pure-read precondition mirror.
int4 RuleOrMask::canApply(const PcodeOp *op,const Funcdata &data) const
{
...
/// \brief Pure-read precondition mirror.
int4 RuleOrConsume::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *outvn = op->getOut();
  int4 size = outvn->getSize();
  if (size > sizeof(uintb)) return 0;
  uintb consume = outvn->getConsume();
  if ((consume & op->getIn(0)->getNZMask()) == 0) return 1;
  if ((consume & op->getIn(1)->getNZMask()) == 0) return 1;
  return 0;
}
...
/// \brief Pure-read precondition mirror.
int4 RuleOrCollapse::canApply(const PcodeOp *op,const Funcdata &data) const
{
  int4 size = op->getOut()->getSize();
  const Varnode *vn = op->getIn(1);
  if (!vn->isConstant()) return 0;
  if (size > sizeof(uintb)) return 0;
  uintb mask = op->getIn(0)->getNZMask();
  uintb val = vn->getOffset();
  return ((mask | val) == val) ? 1 : 0;
}
...
/// \brief Pure-read precondition mirror.
int4 RuleAndOrLump::canApply(const PcodeOp *op,const Funcdata &data) const
{
  if (!op->getIn(1)->isConstant()) return 0;
  const Varnode *vn1 = op->getIn(0);
  if (!vn1->isWritten()) return 0;
  const PcodeOp *op2 = vn1->getDef();
  if (op2->code() != op->code()) return 0;
  if (!op2->getIn(1)->isConstant()) return 0;
  if (op2->getIn(0)->isFree()) return 0;
  return 1;
}
...
/// \brief Pure-read precondition mirror.
int4 RuleNegateIdentity::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *vn = op->getIn(0);
  const Varnode *outVn = op->getOut();
  for (list<PcodeOp *>::const_iterator iter = outVn->beginDescend(); iter != outVn->endDescend(); ++iter) {
    const PcodeOp *logicOp = *iter;
...
/// \brief Pure-read precondition mirror.
int4 RuleShiftBitops::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *constvn = op->getIn(1);
  if (!constvn->isConstant()) return 0;
  const Varnode *vn = op->getIn(0);
  ...
  // INT_MULT additionally requires constvn to be a power of two; conservative skip on that
  if (op->code() == CPUI_INT_MULT && leastsigbit_set(constvn->getOffset()) == -1) return 0;
  return 1;
}
...
/// \brief Pure-read precondition mirror.
int4 RuleRightShiftAnd::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *constVn = op->getIn(1);
  if (!constVn->isConstant()) return 0;
...
/// \brief Pure-read precondition mirror.  functionalEquality is read-only.
int4 RuleEquality::canApply(const PcodeOp *op,const Funcdata &data) const
{
  Varnode *a = const_cast<Varnode *>(op->getIn(0));
  Varnode *b = const_cast<Varnode *>(op->getIn(1));
  return functionalEquality(a, b) ? 1 : 0;
}
...
/// \brief Pure-read precondition mirror.  Quick filter on input being a MULTIEQUAL.
int4 RulePullsubMulti::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *vn = op->getIn(0);
  if (!vn->isWritten()) return 0;
  const PcodeOp *mult = vn->getDef();
...
/// \brief Pure-read precondition mirror.  Quick filter on input being a INDIRECT.
int4 RulePullsubIndirect::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *vn = op->getIn(0);
  if (!vn->isWritten()) return 0;
  if (vn->getSize() > sizeof(uintb)) return 0;
  const PcodeOp *indir = vn->getDef();
...
/// \brief Pure-read precondition mirror.
int4 RuleNotDistribute::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *vn = op->getIn(0);
  if (!vn->isWritten()) return 0;
  const PcodeOp *compop = vn->getDef();
  if (compop == (const PcodeOp *)0) return 0;
  OpCode c = compop->code();
  return (c == CPUI_BOOL_AND || c == CPUI_BOOL_OR) ? 1 : 0;
}
...
/// \brief Pure-read precondition mirror.
int4 RuleAndDistribute::canApply(const PcodeOp *op,const Funcdata &data) const
{
  int4 size = op->getOut()->getSize();
  if (size > sizeof(uintb)) return 0;
  uintb fullmask = calc_mask(size);
  for (int4 i = 0; i < 2; ++i) {
    const Varnode *othervn = op->getIn(1-i);
    if (!othervn->isHeritageKnown()) continue;
    const Varnode *orvn = op->getIn(i);
    const PcodeOp *orop = orvn->getDef();
    if (orop == (const PcodeOp *)0) continue;
    if (orop->code() != CPUI_INT_OR) continue;
    if (!orop->getIn(0)->isHeritageKnown()) continue;
    if (!orop->getIn(1)->isHeritageKnown()) continue;
    uintb othermask = othervn->getNZMask();
    if (othermask == 0) continue;
    if (othermask == fullmask) continue;
    uintb ormask1 = orop->getIn(0)->getNZMask();
    if ((ormask1 & othermask) == 0) return 1;
    uintb ormask2 = orop->getIn(1)->getNZMask();
    if ((ormask2 & othermask) == 0) return 1;
    if (othervn->isConstant()) {
      if ((ormask1 & othermask) == ormask1) return 1;
      if ((ormask2 & othermask) == ormask2) return 1;
    }
  }
  return 0;
}
...
/// \brief Pure-read precondition mirror.
int4 RuleLessOne::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *constvn = op->getIn(1);
  if (!constvn->isConstant()) return 0;
  uintb val = constvn->getOffset();
...
/// \brief Pure-read precondition mirror.
int4 RuleAndPiece::canApply(const PcodeOp *op,const Funcdata &data) const
{
  int4 size = op->getOut()->getSize();
  for (int4 i = 0; i < 2; ++i) {
    const Varnode *piecevn = op->getIn(i);
    if (!piecevn->isWritten()) continue;
    const PcodeOp *pieceop = piecevn->getDef();
    if (pieceop->code() != CPUI_PIECE) continue;
    const Varnode *othervn = op->getIn(1-i);
    uintb othermask = othervn->getNZMask();
    if (othermask == calc_mask(size)) continue;
    if (othermask == 0) continue;
    const Varnode *highvn = pieceop->getIn(0);
    if (!highvn->isHeritageKnown()) continue;
    const Varnode *lowvn = pieceop->getIn(1);
    if (!lowvn->isHeritageKnown()) continue;
    uintb maskhigh = highvn->getNZMask();
    uintb masklow = lowvn->getNZMask();
    if ((maskhigh & (othermask >> (lowvn->getSize()*8))) == 0) {
      if ((maskhigh == 0) && highvn->isConstant()) continue;
      return 1;
    } else if ((masklow & othermask) == 0) {
      if (lowvn->isConstant()) continue;
      return 1;
    }
  }
  return 0;
...
/// \brief Pure-read precondition mirror.
int4 RuleAndZext::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *cvn1 = op->getIn(1);
  if (!cvn1->isConstant()) return 0;
  const Varnode *in0 = op->getIn(0);
  if (!in0->isWritten()) return 0;
  const PcodeOp *otherop = in0->getDef();
  OpCode opc = otherop->code();
  const Varnode *rootvn;
  if (opc == CPUI_INT_SEXT) rootvn = otherop->getIn(0);
  else if (opc == CPUI_PIECE) rootvn = otherop->getIn(1);
  else return 0;
...
/// \brief Pure-read precondition mirror.
int4 RuleAndCompare::canApply(const PcodeOp *op,const Funcdata &data) const
{
  if (!op->getIn(1)->isConstant()) return 0;
  if (op->getIn(1)->getOffset() != 0) return 0;
  const Varnode *andvn = op->getIn(0);
  if (!andvn->isWritten()) return 0;
  const PcodeOp *andop = andvn->getDef();
  if (andop->code() != CPUI_INT_AND) return 0;
  if (!andop->getIn(1)->isConstant()) return 0;
  const Varnode *subvn = andop->getIn(0);
  if (!subvn->isWritten()) return 0;
  OpCode subc = subvn->getDef()->code();
  return (subc == CPUI_SUBPIECE || subc == CPUI_INT_ZEXT) ? 1 : 0;
}
...
/// \brief Pure-read precondition mirror.
int4 RuleDoubleShift::canApply(const PcodeOp *op,const Funcdata &data) const
{
  if (!op->getIn(1)->isConstant()) return 0;
  const Varnode *secvn = op->getIn(0);
  if (!secvn->isWritten()) return 0;
  const PcodeOp *secop = secvn->getDef();
  OpCode opc2 = secop->code();
  if (opc2 != CPUI_INT_LEFT && opc2 != CPUI_INT_RIGHT && opc2 != CPUI_INT_MULT) return 0;
...
/// \brief Pure-read precondition mirror.  Input must be constant comparand against shift/mult/div op.
int4 RuleShiftCompare::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *shiftvn = op->getIn(0);
  const Varnode *constvn = op->getIn(1);
  if (!constvn->isConstant()) return 0;
  if (!shiftvn->isWritten()) return 0;
  const PcodeOp *shiftop = shiftvn->getDef();
  OpCode opc = shiftop->code();
  if (opc != CPUI_INT_LEFT && opc != CPUI_INT_RIGHT && opc != CPUI_INT_MULT && opc != CPUI_INT_DIV) return 0;
  if (!shiftop->getIn(1)->isConstant()) return 0;
  return 1;
}
...
/// \brief Pure-read precondition mirror.  isCseMatch is const + pure.
int4 RuleTrivialArith::canApply(const PcodeOp *op,const Funcdata &data) const
{
  if (op->numInput() != 2) return 0;
  const Varnode *in0 = op->getIn(0);
...
/// \brief Pure-read precondition mirror.  Mirrors applyOp's slot-swap + early-exits.
int4 RuleZextEliminate::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *vn1 = op->getIn(0);
  const Varnode *vn2 = op->getIn(1);
  if (vn2->isWritten() && vn2->getDef()->code() == CPUI_INT_ZEXT) {
    const Varnode *t = vn1; vn1 = vn2; vn2 = t;
  }
  else if (!vn1->isWritten() || vn1->getDef()->code() != CPUI_INT_ZEXT)
    return 0;
  if (!vn2->isConstant()) return 0;
  const PcodeOp *zext = vn1->getDef();
  if (!zext->getIn(0)->isHeritageKnown()) return 0;
  if (vn1->loneDescend() != op) return 0;
  int4 smallsize = zext->getIn(0)->getSize();
  uintb val = vn2->getOffset();
  if ((val >> (8 * smallsize)) != 0) return 0;
  return 1;
}
...
/// \brief Pure-read precondition mirror.
int4 RuleSlessToLess::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *vn = op->getIn(0);
  int4 sz = vn->getSize();
...
/// \brief Pure-read precondition mirror.
int4 RuleZextSless::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *vn1 = op->getIn(0);
  const Varnode *vn2 = op->getIn(1);
  if (vn2->isWritten() && vn2->getDef()->code() == CPUI_INT_ZEXT) {
...
/// \brief Pure-read precondition mirror.
int4 RuleBitUndistribute::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *vn1 = op->getIn(0);
  const Varnode *vn2 = op->getIn(1);
  if (!vn1->isWritten()) return 0;
  ...
  case CPUI_INT_SRIGHT: {
    const Varnode *s1 = vn1->getDef()->getIn(1);
    const Varnode *s2 = vn2->getDef()->getIn(1);
    if (s1->isConstant() && s2->isConstant()) {
      if (s1->getOffset() != s2->getOffset()) return 0;
    } else if (s1 != s2) {
      return 0;
    } else if (s1->isFree()) return 0;
    in1 = vn1->getDef()->getIn(0);
    if (in1->isFree()) return 0;
    in2 = vn2->getDef()->getIn(0);
    if (in2->isFree()) return 0;
...
/// \brief Pure-read precondition mirror.
int4 RuleBooleanNegate::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *constvn = op->getIn(1);
  if (!constvn->isConstant()) return 0;
  uintb val = constvn->getOffset();
  if (val != 0 && val != 1) return 0;
  if (!op->getIn(0)->isBooleanValue(data.isTypeRecoveryOn())) return 0;
  return 1;
}
...
/// \brief Pure-read precondition mirror.
int4 RuleTrivialShift::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *constvn = op->getIn(1);
  if (!constvn->isConstant()) return 0;
  uintb val = constvn->getOffset();
  if (val == 0) return 1;
  if (val < 8 * op->getIn(0)->getSize()) return 0;
...
/// \brief Pure-read precondition mirror.
int4 RuleSignShift::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *constVn = op->getIn(1);
  if (!constVn->isConstant()) return 0;
  uintb val = constVn->getOffset();
  ...
  const Varnode *outVn = op->getOut();
  for (list<PcodeOp *>::const_iterator iter = outVn->beginDescend(); iter != outVn->endDescend(); ++iter) {
    const PcodeOp *arithOp = *iter;
    switch (arithOp->code()) {
    case CPUI_INT_EQUAL:
    case CPUI_INT_NOTEQUAL:
      if (arithOp->getIn(1)->isConstant()) return 1;
      break;
    case CPUI_INT_ADD:
    case CPUI_INT_MULT:
      return 1;
...
/// \brief Pure-read precondition mirror.
int4 RuleTestSign::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *constVn = op->getIn(1);
  if (!constVn->isConstant()) return 0;
  uintb val = constVn->getOffset();
...
/// \brief Pure-read precondition mirror.
int4 RuleShift2Mult::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *constvn = op->getIn(1);
  if (!constvn->isConstant()) return 0;
  ...
  if (val >= 32) return 0;
  const Varnode *vn = op->getOut();
  const PcodeOp *arithop = op->getIn(0)->getDef();
  list<PcodeOp *>::const_iterator desc = vn->beginDescend();
  for (;;) {
    if (arithop != (const PcodeOp *)0) {
      OpCode opc = arithop->code();
      if (opc == CPUI_INT_ADD || opc == CPUI_INT_SUB || opc == CPUI_INT_MULT) return 1;
    }
...
/// \brief Pure-read precondition mirror.
int4 RuleCarryElim::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *vn2 = op->getIn(1);
  if (!vn2->isConstant()) return 0;
  const Varnode *vn1 = op->getIn(0);
...
/// \brief Pure-read precondition mirror.
int4 RuleSubExtComm::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *base = op->getIn(0);
  if (!base->isWritten()) return 0;
  const PcodeOp *extop = base->getDef();
  OpCode c = extop->code();
  if (c != CPUI_INT_ZEXT && c != CPUI_INT_SEXT) return 0;
  const Varnode *invn = extop->getIn(0);
  if (invn->isFree()) return 0;
  return 1;
}
...
/// \brief Pure-read precondition mirror.
int4 RuleShiftAnd::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *cvn = op->getIn(1);
  if (!cvn->isConstant()) return 0;
  const Varnode *shiftin = op->getIn(0);
  if (!shiftin->isWritten()) return 0;
  const PcodeOp *andop = shiftin->getDef();
  if (andop->code() != CPUI_INT_AND) return 0;
  if (!andop->getIn(1)->isConstant()) return 0;
  if (andop->getIn(0)->isFree()) return 0;
  return 1;
}
...
/// \brief Pure-read precondition mirror.
int4 RuleConcatZero::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *vn = op->getIn(1);
  if (!vn->isConstant()) return 0;
  return (vn->getOffset() == 0) ? 1 : 0;
}
...
/// \brief Pure-read precondition mirror.
int4 RuleConcatLeftShift::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *vn2 = op->getIn(1);
  if (!vn2->isWritten()) return 0;
  const PcodeOp *shiftop = vn2->getDef();
  if (shiftop->code() != CPUI_INT_LEFT) return 0;
  if (!shiftop->getIn(1)->isConstant()) return 0;
  int4 sa = shiftop->getIn(1)->getOffset();
  if ((sa & 7) != 0) return 0;
  const Varnode *tmpvn = shiftop->getIn(0);
  if (!tmpvn->isWritten()) return 0;
  const PcodeOp *zextop = tmpvn->getDef();
  if (zextop->code() != CPUI_INT_ZEXT) return 0;
  const Varnode *b = zextop->getIn(0);
...
/// \brief Pure-read precondition mirror.
int4 RuleSubZext::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *subvn = op->getIn(0);
  if (!subvn->isWritten()) return 0;
  const PcodeOp *subop = subvn->getDef();
  if (subop->code() == CPUI_SUBPIECE) {
    const Varnode *basevn = subop->getIn(0);
    if (basevn->isFree()) return 0;
  ...
  if (subop->code() == CPUI_INT_RIGHT) {
    const PcodeOp *shiftop = subop;
    if (!shiftop->getIn(1)->isConstant()) return 0;
    const Varnode *midvn = shiftop->getIn(0);
...
/// \brief Pure-read precondition mirror.
int4 RuleSubCancel::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *base = op->getIn(0);
  if (!base->isWritten()) return 0;
  OpCode opc = base->getDef()->code();
  return (opc == CPUI_INT_ZEXT || opc == CPUI_INT_SEXT || opc == CPUI_INT_AND) ? 1 : 0;
}
...
/// \brief Pure-read precondition mirror.
int4 RuleShiftSub::canApply(const PcodeOp *op,const Funcdata &data) const
{
  if (!op->getIn(0)->isWritten()) return 0;
  const PcodeOp *shiftop = op->getIn(0)->getDef();
  if (shiftop->code() != CPUI_INT_LEFT) return 0;
  const Varnode *sa = shiftop->getIn(1);
  ...
  int4 outsize = op->getOut()->getSize();
  c -= n / 8;
  if (c < 0 || c + outsize > insize) return 0;
  return 1;
}
...
/// \brief Pure-read precondition mirror.
int4 RuleBoolNegate::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *vn = op->getIn(0);
  if (!vn->isWritten()) return 0;
  const PcodeOp *flip_op = vn->getDef();
  for (list<PcodeOp *>::const_iterator iter = vn->beginDescend(); iter != vn->endDescend(); ++iter) {
    if ((*iter)->code() != CPUI_BOOL_NEGATE) return 0;
  }
  bool flipyes;
  OpCode opc = get_booleanflip(flip_op->code(), flipyes);
  return (opc == CPUI_MAX) ? 0 : 1;
}
...
int4 RuleSLess2Zero::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *lvn = op->getIn(0);
  const Varnode *rvn = op->getIn(1);
  if (lvn->isConstant()) {
...
/// \brief Pure-read precondition mirror.
int4 RuleEqual2Zero::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *vn = op->getIn(0);
  const Varnode *addvn;
  ...
  for (list<PcodeOp *>::const_iterator iter = addvn->beginDescend(); iter != addvn->endDescend(); ++iter) {
    if (!(*iter)->isBoolOutput()) return 0;
  }
  const PcodeOp *addop = addvn->getDef();
  if (addop == (const PcodeOp *)0) return 0;
  if (addop->code() != CPUI_INT_ADD) return 0;
  return 1;
}
...
/// \brief Pure-read precondition mirror.
int4 RuleEqual2Constant::canApply(const PcodeOp *op,const Funcdata &data) const
{
  if (!op->getIn(1)->isConstant()) return 0;
  const Varnode *lhs = op->getIn(0);
  if (!lhs->isWritten()) return 0;
  OpCode opc = lhs->getDef()->code();
  if (opc != CPUI_INT_ADD && opc != CPUI_INT_MULT && opc != CPUI_INT_NEGATE) return 0;
  return 1;
}
...
/// \brief Pure-read precondition mirror.
int4 Rule2Comp2Sub::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const PcodeOp *addop = op->getOut()->loneDescend();
  if (addop == (const PcodeOp *)0) return 0;
  return (addop->code() == CPUI_INT_ADD) ? 1 : 0;
}
...
/// \brief Pure-read precondition mirror.
int4 RuleSubNormal::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *shiftout = op->getIn(0);
  if (!shiftout->isWritten()) return 0;
  const PcodeOp *shiftop = shiftout->getDef();
  OpCode opc = shiftop->code();
  if (opc != CPUI_INT_RIGHT && opc != CPUI_INT_SRIGHT) return 0;
  if (!shiftop->getIn(1)->isConstant()) return 0;
  const Varnode *a = shiftop->getIn(0);
  if (a->isFree()) return 0;
  const Varnode *outvn = op->getOut();
...
/// \brief Pure-read precondition mirror.
int4 RuleSignForm::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *sextout = op->getIn(0);
  if (!sextout->isWritten()) return 0;
  const PcodeOp *sextop = sextout->getDef();
  if (sextop->code() != CPUI_INT_SEXT) return 0;
  const Varnode *a = sextop->getIn(0);
  int4 c = op->getIn(1)->getOffset();
...
/// \brief Pure-read precondition mirror.
int4 RuleSignForm2::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *constVn = op->getIn(1);
  if (!constVn->isConstant()) return 0;
  const Varnode *inVn = op->getIn(0);
  ...
  if (!inVn->isWritten()) return 0;
  const PcodeOp *subOp = inVn->getDef();
  return (subOp->code() == CPUI_SUBPIECE) ? 1 : 0;
}
...
/// \brief Pure-read precondition mirror.  At least one INT_AND descendant with mask 1 (1-byte).
int4 RulePopcountBoolXor::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *outVn = op->getOut();
  for (list<PcodeOp *>::const_iterator iter = outVn->beginDescend(); iter != outVn->endDescend(); ++iter) {
    const PcodeOp *baseOp = *iter;
    if (baseOp->code() != CPUI_INT_AND) continue;
    const Varnode *tmpVn = baseOp->getIn(1);
...
/// \brief Pure-read precondition mirror.
int4 RuleXorSwap::canApply(const PcodeOp *op,const Funcdata &data) const
{
  for (int4 i = 0; i < 2; ++i) {
    const Varnode *vn = op->getIn(i);
    if (!vn->isWritten()) continue;
    const PcodeOp *op2 = vn->getDef();
    if (op2->code() != CPUI_INT_XOR) continue;
    const Varnode *othervn = op->getIn(1 - i);
    const Varnode *vn0 = op2->getIn(0);
    const Varnode *vn1 = op2->getIn(1);
    if (othervn == vn0 && !vn1->isFree()) return 1;
    if (othervn == vn1 && !vn0->isFree()) return 1;
  }
  return 0;
}
...
/// \brief Pure-read precondition mirror.
int4 RuleOrCompare::canApply(const PcodeOp *op,const Funcdata &data) const
{
  const Varnode *outvn = op->getOut();
  ...
  if (op->getIn(0)->isFree()) return 0;
  if (op->getIn(1)->isFree()) return 0;
  return 1;
}
