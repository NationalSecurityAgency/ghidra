	...
	private void processSegmentTable(MessageLog log, InformationBlock ib, SegmentTable st,
		...
		try {
			...
			for (int i = 0; i < segments.length; ++i) {
				...
				int offset_int = segments[i].getOffsetShiftAligned();
				int length_int = Short.toUnsignedInt(segments[i].getLength());
				int minalloc_int = Short.toUnsignedInt(segments[i].getMinAllocSize());
				if (minalloc_int == 0) {
					minalloc_int = 0x10000;
				}
				if (length_int > 0) {
					MemoryBlockUtils.createInitializedBlock(program, false, name, addr, fileBytes,
						offset_int, length_int, "", "", r, w, x, log);
				}
				if (length_int < minalloc_int) {
					MemoryBlockUtils.createUninitializedBlock(program, false, name,
						addr.add(length_int), minalloc_int - length_int, "", "", r, w, x, log);
				}
