...
class EditStackReferencePanel extends EditReferencePanel {
	...
	private void buildPanel() {
		...
		JLabel label = new JLabel("Stack Offset:");
    ...
    public boolean setOpIndex(int opIndex) {
		...
		if (opIndex == EditReferencesProvider.MNEMONIC_OPINDEX || !(fromCodeUnit instanceof Instruction)) {
			return false;
		}
    ...
    public boolean applyReference() {
		...
		Function f = program.getFunctionManager().getFunctionContaining(fromCodeUnit.getMinAddress());
		if (f == null) {
		...
		if (offset < minOffset || offset > maxOffset) {
			showInputErr("'Stack Offset' value too " + (offset > 0 ? "large" : "small") + 
					"\nMust be between " + toHexString(minOffset) + " and " + 
					toHexString(maxOffset));
			return false;
		}
