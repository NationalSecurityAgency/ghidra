...
public class NDS32Analyzer extends ConstantPropagationAnalyzer {
	...
	public boolean added(Program program, AddressSetView set, TaskMonitor monitor,
		...
		// Constant-prop loses GP through complex flows (push25 stores etc.);
		// since GP is a global constant on this firmware, a direct walk is
		// the most reliable way to resolve every GP-relative instruction.
		if (markDual && gpAssumptionValue != null) {
			markGpRelativeReferences(program, set, monitor, log);
		}

		// push25/pop25 sleigh stores gp/lp/fp to the stack; constant prop sees
		// `*stack = _gp_1` and adds a DATA ref pointing at the gp base.  Strip
		// those: the store target is the stack slot, not the gp address.
		if (gpAssumptionValue != null) {
			stripPush25PointerRefs(program, set, monitor, log);
		}
	...
	private void stripPush25PointerRefs(Program program, AddressSetView set,
			TaskMonitor monitor, MessageLog log) throws CancelledException {
		...
		for (Instruction insn : iter) {
			...
			if (!mnem.equals("push25") && !mnem.equals("pop25")) {
				continue;
			}
			for (var r : insn.getReferencesFrom()) {
				if (r.getSource() != SourceType.ANALYSIS) {
					continue;
				}
				if (!r.getReferenceType().isData()) {
					continue;
				}
				long target = r.getToAddress().getOffset() & 0xffffffffL;
				if (target == gpBase) {
					refMgr.delete(r);
					removed++;
				}
		...
		if (removed > 0) {
			Msg.info(this, String.format(
				"NDS32 GP markup: post-pass removed %d spurious push25/pop25 -> _gp ref(s)",
				removed));
		}
	}
