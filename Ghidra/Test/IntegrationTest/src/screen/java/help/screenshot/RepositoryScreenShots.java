...
import ghidra.app.merge.listing.AbstractListingMergeManagerTest;
...
//		waitForMergeCompletion();
		...
		mtf.initialize("WallaceSrc", new OriginalProgramModifierListener() {
			...
			public void modifyOriginal(ProgramDB program) throws Exception {
				...
				try {
					...
					setRegValue(pc, addr(program, "004010a0"), addr(program, "004010a3"), regDR0, 0x1234L);
	...
	public void testListingMergeDescriptions() throws Exception {
		...
		ListingPanel[] listingPanels = (ListingPanel[]) getInstanceField("listingPanels", listingMergePanel);
		TitledPanel[] titlePanels = (TitledPanel[]) getInstanceField("titlePanels", listingMergePanel);

		// Bottom of ListingMergePanel is the conflict panel (VariousChoicesPanel or
		...
		addGreyListingDescription(windowLocationOnScreen, listingPanels);
	...
	private void addLockDescription(Image image, Point windowLocationOnScreen, TitledPanel[] titlePanels) {
		@SuppressWarnings("unchecked")
		List<JComponent> titleComps0 = (List<JComponent>) getInstanceField("titleComps", titlePanels[0]);
		...
		@SuppressWarnings("unchecked")
		List<JComponent> titleComps3 = (List<JComponent>) getInstanceField("titleComps", titlePanels[3]);
	...
	private void addShowHeaderDescription(Image image, Point windowLocationOnScreen, TitledPanel[] titlePanels) {
		@SuppressWarnings("unchecked")
		List<JComponent> titleComps0 = (List<JComponent>) getInstanceField("titleComps", titlePanels[0]);
		...
		int buttonY = buttonLocation.y - windowLocationOnScreen.y + (showHeaderButton.getHeight() / 2);
		int arrowHeadX = buttonX - 5;
		int arrowTailX = buttonX - 35;
		String adjustFieldsDescription = "Lets you adjust the fields displayed in all four listings.";
	...
	private void addConflictNumberDescription(Point windowLocationOnScreen, ConflictInfoPanel conflictInfoPanel) {
	...
	private void addAddressDescription(Point windowLocationOnScreen, ConflictInfoPanel conflictInfoPanel) {
		JLabel westLabel = (JLabel) getInstanceField("westLabel", conflictInfoPanel);
		Point westLabelLocation = westLabel.getLocationOnScreen();
	...
	private void addConflictTypeDescription(Point windowLocationOnScreen, ConflictInfoPanel conflictInfoPanel) {
	...
	private void addConflictAreaDescription(Point windowLocationOnScreen, VariousChoicesPanel variousChoicesPanel) {
	...
	private void addUseForAllDescription(Point windowLocationOnScreen, VariousChoicesPanel variousChoicesPanel) {
	...
	private void addApplyDescription(Image image, Point windowLocationOnScreen, Object mergeManagerProvider) {
		...
		mtf.initialize("WallaceSrc", new ProgramModifierListener() {
			...
			public void modifyLatest(ProgramDB program) {
				...
				try {
					...
					Library externalLibrary = symbolTable.createExternalLibrary("Library2", SourceType.USER_DEFINED);
					externalManager.addExtLocation(externalLibrary, "activate", addr(program, "77ba5f22"),
			...
			public void modifyPrivate(ProgramDB program) throws Exception {
				...
				try {
					SymbolTable symbolTable = program.getSymbolTable();
					Library externalLibrary = symbolTable.createExternalLibrary("Library1", SourceType.USER_DEFINED);
					ExternalLocation externalLocation = externalManager.addExtFunction(externalLibrary, "process",
							addr(program, "77ba5f22"), SourceType.USER_DEFINED);
					Function function = externalLocation.getFunction();
					function.addParameter(
							new ParameterImpl("value", new PointerDataType(new IntegerDataType()), program),
							SourceType.USER_DEFINED);
	...
	public void testExternalDataTypeConflict() throws Exception {
		mtf.initialize("WallaceSrc", new OriginalProgramModifierListener() {
			...
			public void modifyLatest(ProgramDB program) {
				...
				ExternalManager externalManager = program.getExternalManager();
				try {
					ExternalLocation externalLocation = externalManager.getUniqueExternalLocation("Library1",
							"activate");
		...
		mtf.initialize("WallaceSrc", new OriginalProgramModifierListener() {
			...
			public void modifyOriginal(ProgramDB program) throws Exception {
				...
				try {
					SymbolTable symbolTable = program.getSymbolTable();
					Library externalLibrary = symbolTable.createExternalLibrary("Library2", SourceType.USER_DEFINED);
					externalManager.addExtLocation(externalLibrary, "process", addr(program, "77ba5f22"),
							SourceType.USER_DEFINED);
			...
			public void modifyLatest(ProgramDB program) throws Exception {
				...
				ExternalManager externalManager = program.getExternalManager();
				try {
					ExternalLocation externalLocation = externalManager.getUniqueExternalLocation("Library2",
							"process");
					Function function = externalLocation.createFunction();

					function.addParameter(new ParameterImpl(null, new Undefined2DataType(), program),
							SourceType.USER_DEFINED);
					function.addParameter(new ParameterImpl(null, new Undefined4DataType(), program),
							SourceType.USER_DEFINED);
					function.addParameter(new ParameterImpl(null, new Undefined4DataType(), program),
							SourceType.USER_DEFINED);
					function.addParameter(new ParameterImpl(null, new Undefined4DataType(), program),
		...
		mtf.initialize("WallaceSrc", new OriginalProgramModifierListener() {
			...
			public void modifyLatest(ProgramDB program) throws Exception {
				...
				try {
					...
					AddressSet body = new AddressSet(addr(program, "0x00401130"), addr(program, "0x00401264"));
					functionManager.createFunction(null, addr(program, "0x00401130"), body, SourceType.USER_DEFINED);
					commit = true;
				} finally {
					program.endTransaction(txId, commit);
				}
			...
			public void modifyPrivate(ProgramDB program) throws Exception {
				...
				try {
					FunctionManager functionManager = program.getFunctionManager();
					AddressSet body2 = new AddressSet(addr(program, "0x00401240"), addr(program, "0x00401264"));
					functionManager.createFunction(null, addr(program, "0x00401240"), body2, SourceType.USER_DEFINED);
					commit = true;
				} finally {
					program.endTransaction(txId, commit);
				}
		...
		mtf.initialize("WallaceSrc", new OriginalProgramModifierListener() {
			...
			public void modifyOriginal(ProgramDB program) throws Exception {
				...
				try {
					SymbolTable symtab = program.getSymbolTable();
					Namespace namespace = getFunction(program, "0x00401130");
					symtab.createLabel(addr(program, "0x0040156c"), "AAA", namespace, SourceType.USER_DEFINED);
					commit = true;
				} finally {
					program.endTransaction(txId, commit);
				}
			...
			public void modifyPrivate(ProgramDB program) throws Exception {
				...
				try {
					SymbolTable symtab = program.getSymbolTable();
					Symbol symbol = symtab.getGlobalSymbol("AAA", addr(program, "0x0040156c"));
					symbol.setNameAndNamespace("ME", program.getGlobalNamespace(), SourceType.USER_DEFINED);
					commit = true;
				} finally {
					program.endTransaction(txId, commit);
				}
		...
		mtf.initialize("WallaceSrc", new ProgramModifierListener() {
			...
			public void modifyLatest(ProgramDB program) throws Exception {
				...
				try {
					...
					symtab.createLabel(addr(program, "0x0040156c"), "BBB", namespace, SourceType.USER_DEFINED);
					commit = true;
				} finally {
					program.endTransaction(txId, commit);
				}
		...
		mtf.initialize("WallaceSrc", new ProgramModifierListener() {
			...
			public void modifyLatest(ProgramDB program) throws Exception {
				...
				try {
					SymbolTable symtab = program.getSymbolTable();
					Namespace namespace = getFunction(program, "0x00401130");
					symtab.createLabel(addr(program, "0x0040156c"), "Foo", namespace, SourceType.USER_DEFINED);
					commit = true;
				} finally {
					program.endTransaction(txId, commit);
				}
			...
			public void modifyPrivate(ProgramDB program) throws Exception {
				...
				try {
					SymbolTable symtab = program.getSymbolTable();
					Namespace namespace = getFunction(program, "0x00401130");
					symtab.createLabel(addr(program, "0x0040156c"), "Bar", namespace, SourceType.USER_DEFINED);
					commit = true;
				} finally {
					program.endTransaction(txId, commit);
				}
	...
	public void testSymbolNamespaceKept() throws Exception {
		mtf.initialize("WallaceSrc", new OriginalProgramModifierListener() {
			...
			public void modifyOriginal(ProgramDB program) {
				...
				try {
					SymbolTable symtab = program.getSymbolTable();
					symtab.createNameSpace(program.getGlobalNamespace(), "FirstNamespace", SourceType.USER_DEFINED);
					symtab.createNameSpace(program.getGlobalNamespace(), "SecondNamespace", SourceType.USER_DEFINED);
					commit = true;
				} catch (Exception e) {
					Assert.fail(e.getMessage());
				} finally {
			...
			public void modifyLatest(ProgramDB program) {
				...
				try {
					SymbolTable symtab = program.getSymbolTable();
					Symbol firstNsSym = getUniqueSymbol(program, "FirstNamespace", program.getGlobalNamespace());
					symtab.removeSymbolSpecial(firstNsSym);
					Namespace secondNamespace = (Namespace) getUniqueSymbol(program, "SecondNamespace",
							program.getGlobalNamespace()).getObject();
					symtab.createLabel(addr(program, "0x0040156c"), "soccer", secondNamespace, SourceType.USER_DEFINED);
					commit = true;
				} catch (Exception e) {
					Assert.fail(e.getMessage());
				} finally {
			...
			public void modifyPrivate(ProgramDB program) {
				...
				try {
					SymbolTable symtab = program.getSymbolTable();
					Symbol secondNsSym = getUniqueSymbol(program, "SecondNamespace", program.getGlobalNamespace());
					symtab.removeSymbolSpecial(secondNsSym);
					Namespace firstNamespace = (Namespace) getUniqueSymbol(program, "FirstNamespace",
							program.getGlobalNamespace()).getObject();
		...
		mtf.initialize("WallaceSrc", new ProgramModifierListener() {
			...
			public void modifyLatest(ProgramDB program) throws Exception {
				...
				try {
					SymbolTable symtab = program.getSymbolTable();
					Namespace namespace = getFunction(program, "0x00401130");
					symtab.createLabel(addr(program, "0x0040114a"), "YOU", namespace, SourceType.USER_DEFINED);
			...
			public void modifyPrivate(ProgramDB program) throws Exception {
				...
				try {
					SymbolTable symtab = program.getSymbolTable();
					Namespace namespace = getFunction(program, "0x00401130");
					symtab.createLabel(addr(program, "0x00401150"), "YOU", namespace, SourceType.USER_DEFINED);
	...
	public void testExternalRefConflict() throws Exception {
		mtf.initialize("WallaceSrc", new OriginalProgramModifierListener() {
			...
			public void modifyOriginal(ProgramDB program) throws Exception {
				...
				try {
					ExternalManager externalManager = program.getExternalManager();
					externalManager.addExtLocation("KERNEL32.DLL", "allocate", null, SourceType.USER_DEFINED);
					externalManager.addExtLocation("KERNEL32.DLL", "stuff", null, SourceType.USER_DEFINED);
					commit = true;
				} finally {
					program.endTransaction(txId, commit);
				}
			...
			public void modifyLatest(ProgramDB program) throws Exception {
				...
				try {
					...
					ExternalManager externalManager = program.getExternalManager();
					ExternalLocation externalLocation = externalManager.getUniqueExternalLocation("KERNEL32.DLL",
							"allocate");
					refMgr.addExternalReference(fromAddress, operandIndex, externalLocation, SourceType.USER_DEFINED,
							RefType.DATA);
					commit = true;
				} finally {
					program.endTransaction(txId, commit);
	...
	public void testRefRegStackConflict() throws Exception {
		mtf.initialize("WallaceSrc", new ProgramModifierListener() {
			...
			public void modifyPrivate(ProgramDB program) throws Exception {
				...
				try {
					ReferenceManager refMgr = program.getReferenceManager();
					refMgr.addStackReference(addr(program, "0x004018e4"), 0, -8, RefType.READ, SourceType.USER_DEFINED);
					commit = true;
				} finally {
					program.endTransaction(txId, commit);
				}
		...
		mtf.initialize("WallaceSrc", new ProgramModifierListener() {
			...
			public void modifyLatest(ProgramDB program) throws Exception {
				...
				try {
					...
					refMgr.addMemoryReference(fromAddress, toAddress, RefType.READ_WRITE, SourceType.USER_DEFINED,
							operandIndex);
					commit = true;
				} finally {
					program.endTransaction(txId, commit);
			...
			public void modifyPrivate(ProgramDB program) throws Exception {
				...
				try {
					...
					refMgr.addMemoryReference(fromAddress, toAddress, RefType.READ, SourceType.USER_DEFINED,
							operandIndex);
					commit = true;
				} finally {
					program.endTransaction(txId, commit);
	...
	public void testRefTypeConflict() throws Exception {
		mtf.initialize("WallaceSrc", new ProgramModifierListener() {
			...
			public void modifyLatest(ProgramDB program) throws Exception {
				...
				try {
					ReferenceManager refMgr = program.getReferenceManager();
					refMgr.addMemoryReference(addr(program, "0x004017db"), addr(program, "0x00401953"), RefType.READ,
							SourceType.USER_DEFINED, 1);
					refMgr.addMemoryReference(addr(program, "0x004017db"), addr(program, "0x0040cbc8"), RefType.DATA,
							SourceType.USER_DEFINED, 1);
			...
			public void modifyPrivate(ProgramDB program) throws Exception {
				...
				try {
					...
					refMgr.addMemoryReference(addr(program, "0x00401810"), addr(program, "0x100190b"), RefType.DATA,
		...
		mtf.initialize("WallaceSrc", new OriginalProgramModifierListener() {
			...
			public void modifyOriginal(ProgramDB program) throws Exception {
				...
				try {
					...
					bookMgr.setBookmark(addr, BookmarkType.ANALYSIS, "Found Code", "Found code from operand reference");
					commit = true;
				} finally {
					program.endTransaction(txId, commit);
				}
			...
			public void modifyLatest(ProgramDB program) throws Exception {
				...
				try {
					BookmarkManager bookMgr = program.getBookmarkManager();
					Address addr = addr(program, "0x004010a0");
					bookMgr.setBookmark(addr, BookmarkType.ANALYSIS, "Found Code", "Latest bookmark @ 0x004010a0");
					commit = true;
				} finally {
					program.endTransaction(txId, commit);
				}
	...
	public void testCommentChangeConflict() throws Exception {
		mtf.initialize("WallaceSrc", new OriginalProgramModifierListener() {
			...
			public void modifyPrivate(ProgramDB program) throws Exception {
				...
				try {
					...
					CodeUnit cu;
					cu = listing.getCodeUnitAt(addr(program, "0x0040156c"));
					cu.setComment(CodeUnit.PRE_COMMENT, "Changed this to a multiple line comment.\n"
		...
		waitForPrompting();
		chooseVerticalCheckBoxes(new String[] { LATEST_CHECK_BOX_NAME, CHECKED_OUT_CHECK_BOX_NAME }, false);
		setToolSize(1100, 750);
		goToListing("0x0040156c");
		scrollHorizontal(30);
	...
	public void testExternalProgramChangeConflict() throws Exception {
		mtf.initialize("WallaceSrc", new OriginalProgramModifierListener() {
			...
			public void modifyOriginal(ProgramDB program) throws Exception {
				...
				boolean commit = false;
				try {
					program.getExternalManager().setExternalPath("ADVAPI32.DLL", "//advapi32.dll", true);
					commit = true;
				} finally {
					program.endTransaction(txId, commit);
				}
	...
	public void testDataTypeSourceConflict() throws Exception {
		...
		mtf.initialize("WallaceSrc", new OriginalProgramModifierListener() {
			...
			public void modifyLatest(ProgramDB program) {
				...
				DataTypeManager dtm = program.getDataTypeManager();
				try {
					SourceArchive sourceArchive = dtm
							.getSourceArchive(myTestArchive.getDataTypeManager().getUniversalID());
					sourceArchive.setName("TestArchiveOne");
					commit = true;
				} finally {
					program.endTransaction(txId, commit);
				}
				SourceArchive sourceArchive = dtm.getSourceArchive(myTestArchive.getDataTypeManager().getUniversalID());
...
//==================================================================================================
	...
	private void removeExternalLibrary(Program program, String libName) {
		...
		while (iter.hasNext()) {
			...
			if (!((ExternalManagerDB) extMgr).removeExternalLocation(loc.getExternalSpaceAddress())) {
	...
	private void setRegValue(ProgramContext pc, Address start, Address end, Register reg, long value)
	...
	protected void changeToRegisterParameter(Function func, int index, Register reg) {
		...
		AddRegisterParameterCommand cmd = new AddRegisterParameterCommand(func, reg, name, dt, index,
				SourceType.USER_DEFINED);
	...
	protected void changeEquate(ProgramDB program, String address, int opIndex, long value, String newName) {
		EquateTable equateTab = program.getEquateTable();
	...
	private void scrollHorizontal(final int percent) {
		runSwing(() -> {
			...
			IndexedScrollPane latestScroller = (IndexedScrollPane) TestUtils.getInstanceField("scroller", latestPanel);
			JScrollPane scrollPane = (JScrollPane) TestUtils.getInstanceField("scrollPane", latestScroller);
